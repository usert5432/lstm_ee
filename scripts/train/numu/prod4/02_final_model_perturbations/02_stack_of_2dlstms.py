"""Train family of final v3 LSTM network with different 2D LSTM configs"""
...
config = join_dicts(
    ...
    # Args:
        ...
        'vars_mod_slice'  : None,
        'outdir'          : \
            'numu/prod4/final_model_perturbations/stack_of_2dlstms/',
    }
)
...
for w in (32, 64, 128, 256):
    search_space += [
        {
            'model' : {
                'kwargs' : { 'lstm2d_spec' : [ (w, 'forward'), ] * N, }
            }
        } for N in range(2, 6)
    ]
    search_space += [
        {
            'model' : {
                'kwargs' : { 'lstm2d_spec' : [ (w, 'backward'), ] * N, }
            }
        } for N in range(1, 6)
    ]
    search_space += [
        {
            'model' : {
                'kwargs' : { 'lstm2d_spec' : [ (w, 'bidirectional'), ] * N, }
            }
        } for N in range(1, 6)
    ]
